...
for iter=1:Max_iteration
    ...
    r=(ub-lb)/dim+((ub-lb)*(iter/Max_iteration)*2);
    ...
    s=2*min(rand)*my_c; % Seperation weight
    a=2*min(rand)*my_c; % Alignment weight
    c=2*max(rand)*my_c; % Cohesion weight
    f=2*max(rand);      % Food attraction weight
